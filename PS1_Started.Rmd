...
lm(Y~V1+V2, data = out)
```
The coefficients are not biased because our coefficients are centered around their true values. 
...
```
This is biased because there is an omitted variable in this regression. 
...
```{r q3}
out$Y_adj <- out$Y - B2*out$V2

lm(Y_adj~V1, data = out)
```
Now it is not biased because our outcome variable has taken away the effect of V2.
